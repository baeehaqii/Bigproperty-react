...
export function PropertySummary({ property }: PropertySummaryProps) {
  const formatPrice = (price: number) => {
    if (price >= 1000000000) {
      return `Rp${(price / 1000000000).toFixed(1).replace(".", ",")} M`
    }
    ...
    <section className="bg-white border rounded-lg p-6 space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-sm">
            <a href="#" className="text-[#0095FF] hover:underline flex items-center gap-1">
              Angsuran mulai dari {property.installment.monthly}/bln
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <span className="px-3 py-1 bg-green-50 text-green-700 text-xs font-semibold rounded">SIAP HUNI</span>
      ...
      <div>
        <h2 className="text-3xl font-bold text-gray-900">
          {formatPrice(property.price.min)} - {formatPrice(property.price.max)}
        </h2>
      ...
      <div className="space-y-3">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              by{" "}
              <a href={`/developer/${property.developer.slug}`} className="text-[#0095FF] hover:underline">
        ...
        <p className="text-sm text-gray-700">
          Kel. {property.location.district}, Kec. {property.location.subDistrict}, Kab. {property.location.city}
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-x-6 gap-y-2 pt-1">
        <div className="flex items-center gap-2 text-sm text-gray-700">
          ...
          <span>{property.specifications.bedrooms} Kamar Tidur</span>
        </div>
        <div className="flex items-center gap-2 text-sm text-gray-700">
          <Maximize className="w-4 h-4 text-gray-500" />
          <span>Luas Tanah {property.specifications.landArea}</span>
        </div>
        <div className="flex items-center gap-2 text-sm text-gray-700">
          <Home className="w-4 h-4 text-gray-500" />
          <span>Luas Bangunan {property.specifications.buildingArea}</span>
        </div>
        <div className="flex items-center gap-2 text-sm text-gray-700">
          <FileText className="w-4 h-4 text-gray-500" />
          <span>{property.specifications.certificateType}</span>
        </div>
      </div>
      ...
      <div className="border-t pt-4 space-y-3">
        ...
        <div className="flex items-start gap-3 p-3 bg-blue-50 rounded-lg border border-blue-100">
          ...
          <div className="flex-1">
            <p className="font-semibold text-sm text-gray-900">CUMA DI PINHOME</p>
            <p className="text-sm text-gray-700 flex items-center gap-1 mt-0.5">
              Emas Batangan hingga 12gr
              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
              </svg>
            </p>
